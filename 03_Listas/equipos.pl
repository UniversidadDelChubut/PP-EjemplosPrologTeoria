...
esGrande(Equipo) :- grandes(Grandes), contiene(Grandes, Equipo).
...
jugoEn(Jugador, Equipo) :- jugador(Jugador, Equipos), contiene(Equipos, Equipo).
...
ganoIntercontinental(Equipo) :- campeonesDelMundo(Campeones), contiene(Campeones, Equipo).
